    ...
    def self.execute(stdout, arguments=[])
      ...
      OptionParser.new do |opts|
        opts.banner = <<-BANNER.gsub(/^          /,'')
          ...
          cssb is a generator of  score files for  csound  to  run
          spatial simulations for any kind of location and any technological setup
          (from mono to stereo to multichannel to high-order ambisonics). +css+ reads
      ...
      cr = Cssb::CsoundRenderer.new(config_root, global_configuration, space_configuration, sound_input_template, movements_template, rao_template)
