...
t(1).room.length = 10; t(1).room.width = 4;
%
%
p1.x = -1; p1.y = 1;
t(2).source =  p1; t(2).result = [ [-1, 9], [5, 1], [-1, -11], [-3, 1] ];
t(2).room.length = 10; t(2).room.width = 4;
%
%
p1.x = 1; p1.y = -1;
t(3).source = p1; t(3).result = [ [1, 11], [3, -1], [1, -9], [-5, -1] ];
t(3).room.length = 10; t(3).room.width = 4;
%
%
p1.x = -1; p1.y = -1;
t(4).source =  p1; t(4).result = [ [-1, 11], [5, -1], [-1, -9], [-3, -1] ];
t(4).room.length = 10; t(4).room.width = 4;
%
t(5).room.length = rand()*37+3; t(5).room.width = rand()*37+3;
p1.x = (t(5).room.width/2)-(0.25*t(5).room.width/2); p1.y = (t(5).room.length/2) - (0.25*t(5).room.length/2); # upper right corner
t(5).source =  p1;
rfw = abs(t(5).room.length/2 - t(5).source.y) + (t(5).room.length/2);
rrw = abs(t(5).room.width/2 - t(5).source.x) + (t(5).room.width/2);
rbw = -abs(-t(5).room.length/2 - t(5).source.y) - (t(5).room.length/2);
...
npos = 10;
for k=6:5+npos
  t(k).room.length = rand()*40+3;
  t(k).room.width = rand()*40+3;
  t(k).source.x = (rand()*t(k).room.width)-(t(k).room.width/2);
  t(k).source.y = (rand()*t(k).room.length)-(t(k).room.length/2);
  t(k).result = zeros(1, 8);
  rfw = abs(t(k).room.length/2 - t(k).source.y) + (t(k).room.length/2);
  rrw = abs(t(k).room.width/2 - t(k).source.x) + (t(k).room.width/2);
  rbw = -abs(-t(k).room.length/2 - t(k).source.y) - (t(k).room.length/2);
...
for k=1:size(t, 2)
  r = refl_1st_order(t(k).source, [t(k).room.length t(k).room.width]);
